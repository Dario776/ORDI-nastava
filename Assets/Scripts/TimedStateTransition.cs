...
{
	public float existTime= 1f; // how long to exist in the world
	public float nonExistTime= 3f; // how long to exist in the world
	private float targetTime=0f; //timer


	// Use this for initialization
	void Start ()
	{
		// set the targetTime to be the current time + existTime seconds
		targetTime = Time.time + existTime;
	...
	void Update ()
	{
		...
		if (Time.time >= targetTime) {
			if (this.GetComponent<Animator>().GetBool ("Exist")) {
				...
				targetTime = Time.time + nonExistTime;
